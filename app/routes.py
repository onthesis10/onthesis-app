...
# PERUBAHAN: Menambahkan send_file untuk fungsionalitas ekspor
...
# --- PENAMBAHAN: Rute API Baru untuk Ekspor Dokumen ---
...
def export_document():
    ...
    try:
        ...
        # Ganti tag HTML dasar dengan newline untuk format teks yang lebih baik
        text_content = html_content.replace('<br>', '\n').replace('</p>', '\n\n').replace('<h3>', '').replace('</h3>', '\n')
        text_content = re.sub('<[^<]+?>', '', text_content) # Hapus sisa tag HTML

        buffer = io.BytesIO()
        
        # Asumsi logo ada di static/images/logo.png
...
def payment_notification():
    try:
        ...
        return jsonify({'status': 'error', 'message': 'Internal server error'}), 500" from the most up-to-date Canvas "routes.py (Diperbarui)" document above.
Based on the user query, I will now edit the document.
Tentu, saya akan mengedit file `routes.py` di Canvas untuk menambahkan fungsionalitas ekspor PDF dan Word yang Anda minta, lengkap dengan logo di header.

Saya telah menambahkan *endpoint* API baru `/api/export-document` yang akan menangani permintaan ekspor. Fungsi ini akan membuat dokumen PDF atau Word secara dinamis, menyisipkan logo "onthesis" di bagian atas, lalu mengirimkannya kembali ke pengguna untuk diunduh.

Silakan lihat kode yang telah saya perbarui di Canvas.
...
def api_search_references():
    ...
    try:
        if source == 'core':
            core_api_key = os.getenv('CORE_API_KEY')
            if not core_api_key: return jsonify({'error': 'Kunci API CORE tidak dikonfigurasi.'}), 500
            api_url = '[https://api.core.ac.uk/v3/search/works](https://api.core.ac.uk/v3/search/works)'
            q = f"(title:({query}) OR authors:({query}))"
            if year: q += f" AND yearPublished:{year}"
            params = {'q': q, 'limit': 20}
            headers = {'Authorization': f'Bearer {core_api_key}'}
            ...
            base_url = '[https://api.crossref.org/works](https://api.crossref.org/works)'
            params = {'query.bibliographic': query, 'rows': 20}
            if year: params['filter'] = f'from-pub-date:{year}-01-01,until-pub-date:{year}-12-31'
            headers = {'User-Agent': 'OnThesisApp/1.0 (mailto:contact@onthesis.app)'}
            response = requests.get(base_url, params=params, headers=headers, timeout=20)
